    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={ps.cycleCard}>
        ...
        <View style={ps.focusChipRow}>
          {(planType === 'recovery'
            ? ['MOBILITY', 'SLEEP', 'NUTRITION']
            : planType === 'progressive'
            ? ['STRENGTH', 'RUCK', 'CARDIO']
            : ['STRENGTH', 'ENDURANCE', 'BALANCE']
          ).map((chip) => (
            <View key={chip} style={ps.focusChip}>
              <Text style={ps.focusChipText}>{chip}</Text>
            </View>
          ))}
        </View>
      </View>
          ...
          {days.map((d, i) => {
            ...
            return (
              <View key={d.day} style={[ps.tempoDay, isToday ? ps.tempoDayToday : null]}>
                ...
                {dotColor ? (
                  <View style={[ps.tempoDot, { backgroundColor: dotColor }]} />
                ) : (
                  <View style={ps.tempoDotEmpty} />
                )}
              </View>
            );
          })}
